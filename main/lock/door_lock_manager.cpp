...
CHIP_ERROR BoltLockManager::InitLockState()
{
    ...
    // Enforce default locked state on boot if it's unlocked or uninitialized
    if (state.IsNull() || state.Value() == DoorLock::DlLockState::kUnlocked) {
        state.SetNonNull(DoorLock::DlLockState::kLocked);
        DoorLockServer::Instance().SetLockState(endpointId, DoorLock::DlLockState::kLocked);
    }
