...
public:
    ...
    size_t get(uint8_t* buffer, size_t buffer_size) const {
        ...
        if (buffer_size < required_size) {
            printf("Buffer too small for packing. Required: %zu, Available: %zu\n", required_size, buffer_size);
            return 0;
        }
    ...
    void parse(const uint8_t* buffer, size_t buffer_size) {
        ...
        while (current_pos < end_pos) {
            ...
            if (current_pos + 1 > end_pos) {
                printf("Unpack error: Insufficient data for length byte (tag: 0x%02X)\n", tag);
                return;
            }
            size_t len = 0;
            ...
            if (use_ber_length_encoding && (len_byte & 0x80)) {
                ...
                if (len_bytes_count == 0 || len_bytes_count > sizeof(size_t)) {
                    printf("Unpack error: Unsupported BER length form (tag: 0x%02X, len_byte: 0x%02X)\n", tag, len_byte);
                    return;
                }
                if (current_pos + len_bytes_count > end_pos) {
                    printf("Unpack error: Insufficient data for BER long length value (tag: 0x%02X)\n", tag);
                    return;
                }
                len = 0;
            ...
            if (current_pos + len > end_pos) {
                printf("Unpack error: Insufficient data for value (tag: 0x%02X, length: %zu)\n", tag, len);
                return;
            }
